...
let BYBIT_TOTAL_MONEY = 0;
let BYBIT_TOTAL_PNL = 0;

// global flag used by redirect logic
window.HAS_FUTURE_DATA = window.HAS_FUTURE_DATA || false;

const server_bybit = "192.168.1.7";
const url_bybit = `http://${server_bybit}:1704/linux/execute`;
const headers_bybit = {
...
async function call3ApisAtOnce() {
    ...
    const requests = [
        fetch(url_bybit, {
            ...
            body: JSON.stringify({
                command: "cd /var/lib/ApiGateway/blockchain/bybit && source .venv/bin/activate && python vi_the_future.py",
            }),
        }),
        fetch(url_bybit, {
            method: "POST",
            headers: headers_bybit,
            body: JSON.stringify({
                command: "cd /var/lib/ApiGateway/blockchain/bybit && source .venv/bin/activate && python balance.py",
            }),
        }),
    ];
...
(async () => {
    ...
    document.getElementById("money_goc_bybit").innerHTML = "(Tiền: " + spot.total?.equity?.toFixed(1) + "$)";

    BYBIT_TOTAL_MONEY = spot.total?.equity;
    BYBIT_TOTAL_PNL = spot.total?.unrealized_pnl;

    sessionStorage.setItem('BYBIT_TOTAL_MONEY', BYBIT_TOTAL_MONEY);
    sessionStorage.setItem('BYBIT_TOTAL_PNL', BYBIT_TOTAL_PNL);
    
    if (spot.total?.unrealized_pnl >= 0) {
        document.getElementById("lai_lo_bybit").innerHTML = "(Lãi: " + spot.total?.unrealized_pnl?.toFixed(1) + "$)";
        document.getElementById("lai_lo_bybit").style.color = "#00ff00";
        document.getElementById("lai_lo_bybit").style.textShadow = "0 0 5px #00ff00";
    } else {
        document.getElementById("lai_lo_bybit").innerHTML = "(Lỗ: " + Math.abs(spot.total?.unrealized_pnl)?.toFixed(1) + "$)";
        document.getElementById("lai_lo_bybit").style.color = "#dc2626";
        document.getElementById("lai_lo_bybit").style.textShadow = "0 0 5px #dc2626";
    }
    ...
    document.getElementById("bybit_body_row").innerHTML = futureHTML;
